...
{
    [Binding]
    public class UpdateIndividualDeserializationValidationStepDefinitions
    {
        private readonly ScenarioApiContext _apiContext;
        private Guid _customerId;
        private Dictionary<string, object?>? _rawPayload;

        public UpdateIndividualDeserializationValidationStepDefinitions(ScenarioApiContext apiContext)
        {
            _apiContext = apiContext;
        }

        [Given("a customer exists for update individual missing fields request")]
        public async Task GivenACustomerExistsForUpdateIndividualMissingFieldsRequest(Table table)
        {
            var requestValues = ParseExpectedTable(table);
            AllureJson.AttachObject("Update individual missing fields customer setup table", requestValues, _apiContext.JsonOptions);
            ...
            AllureJson.AttachObject(
                "Create customer setup payload for update individual missing fields flow",
                createRequest,
        ...
        [Given("I have an update individual request payload missing the individual email and phone")]
        public void GivenIHaveAnUpdateIndividualRequestPayloadMissingTheIndividualEmailAndPhone(Table table)
        {
            var requestValues = ParseExpectedTable(table);
            AllureJson.AttachObject("Incomplete update individual request table", requestValues, _apiContext.JsonOptions);
            ...
            _rawPayload = new Dictionary<string, object?>(StringComparer.OrdinalIgnoreCase)
            {
                ["individual"] = new Dictionary<string, object?>(StringComparer.OrdinalIgnoreCase)
                {
                    ...
                    ["billingAddress"] = BuildRawAddress(requestValues, "Individual.Billing"),
                    ["shippingAddress"] = BuildRawAddress(requestValues, "Individual.Shipping")
                }
            };

            AllureJson.AttachObject(
                "Incomplete update individual raw request JSON dictionary structure",
                _rawPayload,
                _apiContext.JsonOptions);
        }

        [When("I submit the incomplete update individual request")]
        public async Task WhenISubmitTheIncompleteUpdateIndividualRequest()
        {
            ...
            AllureJson.AttachRawJson($"Update individual deserialization exception response JSON ({(int)_apiContext.Response.StatusCode})", body);
        }

        [Then("the update individual response indicates a json deserialization failure")]
        public async Task ThenTheUpdateIndividualResponseIndicatesAJsonDeserializationFailure(Table table)
        {
            var expected = ParseExpectedTable(table);
            AllureJson.AttachObject("Expected update individual deserialization error constraints", expected, _apiContext.JsonOptions);
            ...
            problemDetails.Instance.ShouldBe($"/customers/{_customerId}/individual");
        ...
        private static IReadOnlyCollection<string> GetRequiredValues(IReadOnlyDictionary<string, List<string>> values, string key)
        {
            ...
            {
                throw new InvalidOperationException($"Missing '{key}' value in update individual missing fields data table.");
            }
