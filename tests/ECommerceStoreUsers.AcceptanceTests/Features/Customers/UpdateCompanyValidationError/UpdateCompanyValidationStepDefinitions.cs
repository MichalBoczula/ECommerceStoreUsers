...
namespace ECommerceStoreUsers.AcceptanceTests.Features.Customers.UpdateCompanyValidationError
{
    ...
    public class UpdateCompanyValidationStepDefinitions
    {
        ...
        public async Task GivenACustomerWithACompanyExistsForUpdateCompanyValidationRequest(Table table)
        {
            var requestValues = ParseExpectedTable(table);
            AllureJson.AttachObject("Update company validation setup customer request table", requestValues, _apiContext.JsonOptions);
            ...
            AllureJson.AttachObject(
                "Create customer with company setup request contract for update company validation",
                createRequest,
            ...
            AllureJson.AttachRawJson($"Create customer with company setup response contract for update company validation ({(int)response.StatusCode})", body);
        ...
        public async Task ThenUpdatingTheCompanyDataFailsWithValidationErrors(Table table)
        {
            ...
            AssertValidationError(
                problemDetails,
                GetRequiredValue(expected, "TaxIdName"),
                GetRequiredValue(expected, "TaxIdMessage"),
                GetRequiredValue(expected, "TaxIdEntity"));

            AssertValidationError(
                problemDetails,
                GetRequiredValue(expected, "CompanyNameName"),
                GetRequiredValue(expected, "CompanyNameMessage"),
                GetRequiredValue(expected, "CompanyNameEntity"));
        ...
        private static string GetRequiredValue(IReadOnlyDictionary<string, string> values, string key)
        {
            ...
            {
                throw new InvalidOperationException($"Missing '{key}' validation key value in update company data table.");
            }
